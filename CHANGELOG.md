...
- All custom Nunjucks' tags and block tags are now wrapped in a Promise, making it no longer necessary to denote whether a tag's function is async or not. You are safe to return sync or async values without any extra work.
...
- Custom filters, tags and block tags are now passed in the configuration nested under a `nunjucks` object. This namespaces them a bit better and sidesteps a scenario where we keep adding more and more things with `nunjucks*` prefixes.
