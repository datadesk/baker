...
class NunjucksEngine extends BaseEngine {
  ...
  addCustomTag(name, fn) {
    ...
    class CustomTag {
      ...
      run(...args) {
        const addWatchFile = (fp) => addDependency(fp, this.engine.inputPath);

        if (isAsync) {
          const resolve = args.pop();
          const [context, ...params] = args;

          fn.call({ addWatchFile, page: context.ctx.page }, ...params)
            .then((value) => {
          ...
          const value = fn.call(
            { addWatchFile, page: context.ctx.page },
            ...params
          );
    ...
    this.env.addExtension(name, new CustomTag(this));
  }
