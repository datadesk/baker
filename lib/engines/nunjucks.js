...
import { relative, join, resolve, parse } from 'path';
...
export class NunjucksEngine extends BaseEngine {
  constructor({ layouts, createPages, postprocess, ...args }) {
    super(args);
    ...
    // a function to run against the rendered HTML
   ...
   * @param {import('../types').NunjucksFilter} fn The function for the filter
   */
  ...
  /**
   * @param {Record<string, import('../types').NunjucksFilter>} obj
   */
  ...
  /**
   * @param {string} name
   * @param {import('../types').NunjucksTag} fn
   */
  addCustomTag(name, fn) {
    this.env.addExtension(name, {
      tags: [name],
  ...
  /**
   * @param {Record<string, import('../types').NunjucksTag>} obj
   */
  addCustomTags(obj) {
    for (const [name, fn] of Object.entries(obj)) {
  ...
  /**
   * @param {string} name
   * @param {import('../types').NunjucksBlockTag} fn
   */
  addCustomBlockTag(name, fn) {
    this.env.addExtension(name, {
      ...
      async run(_, ...args) {
        ...
        // the body is always the next to last item
        const body = args.pop();

        try {
          // run the function
          const value = await fn(body(), ...args);
  ...
  /**
   * @param {Record<string, import('../types').NunjucksBlockTag>} obj
   */
  addCustomBlockTags(obj) {
    for (const [name, fn] of Object.entries(obj)) {
      this.addCustomBlockTag(name, fn);
    }
  }
  ...
  static async renderOnly(filepath, context) {
    ...
    // create the template
    const template = new Template(raw, _resolve(filepath));
