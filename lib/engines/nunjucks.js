...
export class NunjucksEngine extends BaseEngine {
  constructor({ layouts, createPages, ...args }) {
    ...
    const fileLoader = new nunjucks.FileSystemLoader(
      [this.layouts, this.input],
      {
        noCache: true,
      }
    );
    ...
    const nodeLoader = new nunjucks.NodeResolveLoader({ noCache: true });

    // the prepared nunjucks environemnt
    this.env = new nunjucks.Environment([fileLoader, nodeLoader], {
      autoescape: false,
      dev: true,
  ...
  async compile(file) {
    ...
    // create the template
    const template = new nunjucks.Template(raw, this.env, resolve(file));
    ...
    class CustomTag {
      ...
      run(...args) {
        if (isAsync) {
          ...
          fn.call({ addDependency, page: context.ctx.page }, ...params)
            .then((value) => {
          ...
          const value = fn.call(
            { addDependency, page: context.ctx.page },
            ...params
          );
