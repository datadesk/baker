...
class NunjucksEngine extends BaseEngine {
  constructor({ layouts, generatePages, ...args }) {
    super(args);
  ...
  collectDynamicRenders() {
    ...
    const collect = (inputPath, outputPath, localContext) => {
      renders.push([
        path.join(this.layouts, inputPath),
        outputPath,
        localContext,
      ]);
    };
  ...
  async build() {
    ...
    try {
      ...
      if (this.generatePages) {
        ...
        this.generatePages(collect, this.context);
        await Promise.all(renders.map((props) => this.outputFile(...props)));
      }
    } catch (err) {
      throw err;
    }
