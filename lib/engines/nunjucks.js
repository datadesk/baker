...
class NunjucksEngine extends BaseEngine {
  constructor({ layouts, createPages, ...args }) {
    ...
    this.fileLoader = new FileSystemLoader([this.layouts, this.input], {
      noCache: false,
    });

    // handles loading template files from node_modules
    this.nodeLoader = new NodeResolveLoader({ noCache: false });

    // the prepared nunjucks environemnt
    this.env = new Environment([this.fileLoader, this.nodeLoader], {
      autoescape: false,
      dev: true,
    ...
    this.env.on('load', (_, { path: templatePath }) => {
      console.log('on.load:', templatePath);
  ...
  whenFileChanges(path) {
    this.fileLoader.emit('update', this.fileLoader.pathsToNames[path], path);
    this.nodeLoader.emit('update', this.nodeLoader.pathsToNames[path], path);
  }
  ...
  async outputFile(inputPath, outputPath, localContext = {}) {
    ...
    if (isProductionEnv) {
      content = this.minifier(content, { collapseWhitespace: true });
    ...
    class CustomTag {
      ...
      run(_, ...args) {
        if (isAsync) {
          ...
          fn(...args, (err, value, dependencies = []) => {
            if (err) callback(err);
