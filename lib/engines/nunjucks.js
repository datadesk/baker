...
class NunjucksEngine extends BaseEngine {
  ...
  getOutputPath({ base, ext, input, name }) {
    // if the file is a txt, we build it in place
    if (ext === '.txt') {
        return input;
    }
    // we always use "pretty" URLs for html and njk files, so we alter the pathname if it is index.html
    const pathname =
  ...
  async outputFile(inputPath, outputPath, localContext = {}) {
    ...
    if (isProductionEnv) {
      content = this.minifier(content, { collapseWhitespace: parts.ext != '.txt' });
