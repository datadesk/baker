...
class NunjucksEngine extends BaseEngine {
  ...
  async compile(file) {
    ...
    // parse out any frontmatter
    const { content, data } = matter(raw, {
      engines: { aml: archieml.load.bind(archieml) },
    });

    // create the template
    const template = new Template(content, this.env, path.resolve(file));

    return function render(context) {
      return new Promise((resolve, reject) => {
        template.render({ ...context, ...data }, (err, text) => {
          if (err) {
  ...
  async render(file) {
    ...
    const currentPath = path.join(this.pathPrefix, dir, pathname);
