...
class AssetsEngine extends BaseEngine {
  ...
  async getOutputPath({ content, dir, ext, name }) {
    ...
    // if we're in production and this is an image or font, hash it, otherwise skip
    if (
      isProductionEnv &&
      (validImageExtensions.includes(normalizedExt) ||
        validFontExtensions.includes(normalizedExt))
    ) {
  ...
  async render(file) {
    ...
    if (validImageExtensions.includes(ext.toLowerCase())) {
