...
export class RollupEngine extends BaseEngine {
  ...
  async buildEmbed(output) {
    ...
    const embedBundle = await rollup({
      input: 'lib/components/baker-embed.js', // This will be created dynamically in your function
      plugins: [
    ...
    const webComponentBundle = await rollup({
      input: 'lib/components/web-component.js', // This will be created dynamically in your function
      plugins: [
        nodeResolve({ extensions: ['.js'], browser: true }),
        commonjs(),
        replace({
