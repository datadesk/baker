...
class RollupEngine extends BaseEngine {
  ...
  generateInputOptions(input, replaceValues, nomodule = false) {
    ...
    const config = {
      input,
      plugins: [
        !nomodule && prependEntry({ content: `import "${polyfillsDynamicImport}";\n` }),
        ...
        svelte({ css: false, preprocess: svelteConfig.preprocess }),
        babel({
  ...
  async generateBundle(input, entrypoints, replaceValues, nomodule = false) {
    ...
    for (const entrypoint of entrypoints) {
      const { name } = path.parse(entrypoint);
      const chunk = info.output.find((chunk) => entrypoint === chunk.facadeModuleId);
