...
import { manifestPlugin } from '../rollup-plugins/manifest-plugin.js';
...
export class RollupEngine extends BaseEngine {
  ...
  async build() {
    ...
    console.log('modernInput', modernInput)
  ...
  async buildEmbed(output) {
    // baker-embed.js
    console.log('embed output', resolve(process.env.PWD, 'lib/components/baker-embed.js'))
    const embedBundle = await rollup({
      input: resolve(process.env.PWD, 'lib/components/baker-embed.js'),
      plugins: [
        ...
        terser({
          ecma: isProductionEnv ? 5 : 8,
          safari10: true,
        }),
      ],
    });

    await embedBundle.write({
      dir: output,
      format: 'iife',
      name: 'BakerEmbed',
    });
    ...
    const webComponentBundle = await rollup({
      input: resolve(process.env.PWD, 'lib/components/web-component.js'),
      plugins: [
        nodeResolve({ extensions: ['.js'], browser: true }),
        commonjs(),
        replace({
    ...
    await webComponentBundle.write({
      entryFileNames: isProductionEnv ? '[name].[hash].js' : '[name].js',
      dir: output,
      format: 'iife',
      name: 'BakerWebComponent',
    });
  }
}
