...
const { concatEntry } = require('../rollup-plugins/concat-entry');
...
class RollupEngine extends BaseEngine {
  ...
  generateModernInput(entrypoints) {
    ...
    return entrypoints.reduce((entries, entrypoint) => {
      ...
      entries[name] = [
        !isProductionEnv && require.resolve('mini-sync/client'),
        polyfillsDynamicImport,
        entrypoint,
      ].filter(Boolean);
    ...
    const config = {
      ...
      plugins: [
        ...
        babel({
          ...
          extensions,
          presets: [
            !nomodule && require.resolve('@babel/preset-modules'),
            nomodule && [
          ...
          plugins: [
            ...
            !nomodule && require.resolve('@babel/plugin-syntax-dynamic-import'),
  ...
  async build() {
    ...
    const modern = await this.generateBundle(
      modernInput,
      entrypoints,
      replaceValues
    );
    ...
    if (isProductionEnv) {
      ...
      for (const entrypoint of entrypoints) {
        ...
        const legacy = await this.generateBundle(
          legacyInput,
          entrypoints,
          replaceValues,
          true
