...
// local
const { noop, outputFile, union } = require('../utils');
...
class BaseEngine {
  ...
  constructor({ domain, input, output, pathPrefix, staticRoot }) {
    ...
    /**
     * A mapping of what an entrypoint depends on.
     * @type {Map<string, Set<string>>}
     */
    this.mappedDependencies = new Map();

    /**
     * A mapping of what entrypoints a file is a dependent of.
     *  @type {Map<string, Set<string>>}
     */
    this.mappedDependents = new Map();
  ...
  /**
   * Called whenever the watcher detects a file change.
   *
   * @param {string} path
   */
  whenFileChanges(path) {}
  ...
  addDependency(file, importer) {
    if (this.mappedDependencies.has(importer)) {
      this.mappedDependencies.get(importer).add(file);
    } else {
      this.mappedDependencies.set(importer, new Set([file]));
    }

    if (this.mappedDependents.has(file)) {
      this.mappedDependents.get(file).add(importer);
    } else {
      this.mappedDependents.set(file, new Set([importer]));
    }
  ...
  resetDependencies() {
    // grab the existing ones
    const previousDependencies = this.dependencies;

    // reset the dependencies
    this.dependencies = new Set();

    // reset the mappedDependencies
    this.mappedDependencies = new Map();
  ...
  watch(fn = noop) {
    ...
    if (this.name === 'nunjucks') {
      ...
      console.log('mappedDependencies:', this.mappedDependencies);
      console.log('mappedDependents:', this.mappedDependents);
    }
    ...
    const onChange = async (path) => {
      if (this.whenFileChanges) {
        this.whenFileChanges(path);
      }

      // const previousDependencies = this.resetDependencies();
      // console.log('previousDependencies:', previousDependencies);

      // get all the entrypoints this file is used in
      const inputs = Array.from(this.mappedDependents.get(path));

      try {
        let results = [];

        // we do this sequentially on purpose
        for (const input of inputs) {
          results.push(await this.outputFile(input));
        }
        ...
        // for (const file of previousDependencies) {
    ...
    ['add', 'change', 'unlink'].forEach(addEvent);
  }
}
