// native
import { join, format, relative, parse } from 'path';
...
export class BaseEngine {
  ...
  addDependency(file, importer) {
    ...
    if (this.mappedDependents.has(importer)) {
      this.mappedDependents.get(importer).add(file);
    } else {
      this.mappedDependents.set(importer, new Set([file]));
    }
  ...
  watch(fn = noop) {
    ...
    if (this.name === 'nunjucks')
      console.log(
        this.dependencies,
        this.mappedDependencies,
        this.mappedDependents
      );
