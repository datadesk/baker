...
export function getBasePath(domain) {
  // Local dev can use relative paths
  if (!isProductionEnv) return '/';

  // Dev and Prod deployments need to use absolute paths
  const basePath = process.env.BAKER_BASE_PATH || domain;
  return `${basePath}/${process.env.BAKER_PATH_PREFIX}/`;
}
