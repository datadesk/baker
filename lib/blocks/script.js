...
export function createScriptBlock(rollup) {
  return function script(entry, shouldPreload = false) {
    ...
    const cssEntry = rollup.getManifestEntry('css')[entry];
    const modernEntry = rollup.getManifestEntry('modern')[entry];
    const legacyEntry = rollup.getManifestEntry('legacy')[entry];

    console.log({
      cssEntry, modernEntry, legacyEntry
    })
