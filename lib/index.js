...
export class Baker extends EventEmitter {
  ...
  async bakeEmbeds() {
    try {
      ...
      const webComponentHashRegex = /__WEB_COMPONENTS_HASH__/g;
      const bakerPathPrefixRegex = /__BAKER_PATH_PREFIX__/g;

      // build all the necessary javascript files for web components
      await this.rollup.buildEmbed(outputDir);

      // replace all the necessary strings in the baker-embed.js file and web-component.js file
      let [manifestJson, bakerEmbedCode] = await Promise.all([
        fs.readFile(manifestJsonPath, 'utf8'),
        fs.readFile(bakerEmbedPath, 'utf8')
      ]);
      const manifest = JSON.parse(manifestJson);
      const webComponentsHashedFile = manifest['web-component.js'];

      bakerEmbedCode = bakerEmbedCode.replace(webComponentHashRegex, webComponentsHashedFile);
