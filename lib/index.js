  ...
  async bakeEmbeds(filenames) {
    ...
    for (const filename of filenames) {
      try {
        ...
        fileMapper[bakerId] = `example/_dist/embeds/${bakerId}/index.html`;
      } catch (err) {
    ...
    const embedScriptContent = `
      ...
      module.exports = class BakerComponent extends HTMLElement {
        constructor() {
          super();

          this.fileMapper = ${JSON.stringify(fileMapper)};
        }
        ...
        static register() {
          ...
          if (!window.customElements.get(BakerComponent.customElementName)) {
            console.log('registering...')
            window.customElements.define(
              BakerComponent.customElementName,
              BakerComponent
            );
          }

          // Wait for it
          return window.customElements.whenDefined(BakerComponent.customElementName);
        }
      }
      `;
...
/**
 * 1. observe datawarpper behavior
 * 2.
 */
