...
// local
import { bakerPathPrefix, isProductionEnv } from './env.js';
...
export class Baker extends EventEmitter {
   ...
   */
  async bakeEmbeds() {
    const pathPrefix = bakerPathPrefix(basename(this.input));
    const webComponentsHashRegex = new RegExp('__WEB_COMPONENTS_HASH__', 'g');
    const bakerPathPrefixRegex = new RegExp('__BAKER_PATH_PREFIX__', 'g');

    try {
      const outputDir = join(this.output, 'embeds');

      // build all the necessary javascript files for web components
      await this.rollup.buildEmbed(outputDir);

      // handle reference to the hashed web components file in the baker-embed.js
      ...
      const webComponentsHashedFile = manifest['web-component.js'];

      // read the baker-embed.js file
      const bakerEmbedPath = join(outputDir, 'baker-embed.js');
      let bakerEmbedCode = await fs.readFile(bakerEmbedPath, 'utf8');

      // replace the __WEB_COMPONENTS_HASH__ with the hashed web components file in the baker-embed.js
      bakerEmbedCode = bakerEmbedCode.replace(webComponentsHashRegex, webComponentsHashedFile);
      bakerEmbedCode = bakerEmbedCode.replace(bakerPathPrefixRegex, pathPrefix);

      // handle baker_path_prefix
      ...
      webComponentJsCode = webComponentJsCode.replace(bakerPathPrefixRegex, pathPrefix);

      await Promise.all([
        fs.writeFile(webComponentJsPath, webComponentJsCode),
        fs.writeFile(bakerEmbedPath, bakerEmbedCode),
        fs.unlink(manifestJsonPath)
      ]);
    } catch (err) {
      console.error(`Error in bakeEmbeds: ${err.message}`);
      throw err;
    }
  }
  ...
  async bake() {
    ...
    const embedFiles = await glob(join(this.output, 'embeds/*', '*.html'), {
      ignore: 'node_modules/**',
    });
