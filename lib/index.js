...
export class Baker extends EventEmitter {
  ...
  async bakeEmbeds(context, filename) {
    // pass context to rolleup
    this.rollup.context = context;
    this.nunjucks.context = context;

    compiled_file = await this.nunjucks.compile(test)



    // check the embeds folder
    // this.nunjucks.compile and inject into a custom element
    // console.log('TESTING', this.embeds);
    // go into this folder and find the related files

    // call glob this
    // const jsfiles = await glob('**/*.js', { ignore: 'node_modules/**' })
    // let tests = await glob(join(this.embeds, '*.html'), {
    //   ignore: 'node_modules/**',
    // });
    // console.log(tests);

    // // loop through teglobst and get output of nunjucks.compile
    // for (const test of tests) {
    //   console.log('this.nunjucks printout is', this.nunjucks)
    //   // this.nunjucks.compile refers to the function compile in nunjucks.js file
    //   let compiled_file = await this.nunjucks.compile(test);
  ...
  async bake() {
    ...
    if (this.embeds) {
      ...
      let files = await glob(join(this.embeds, '*.html'), {
        ignore: 'node_modules/**',
      });
      for (file of files) {
        this.bakeEmbeds(embedData, file);
      }
    }
    ...
    // // check to see if there are embeds and, if so, process them
    // await this.bakeEmbeds(); commented out for now
