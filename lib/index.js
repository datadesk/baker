...
export class Baker extends EventEmitter {
  ...
  prepareCrosswalk() {
    ...
    return function setCrosswalkDefaultPaths(
      ...
      const fixCrosswalkPaths = (assetType, obj, allowedExtensions, requiredExtensions = []) => {
      ...
      if (img) {
        preppedData.img = fixCrosswalkPaths('img', img, CROSSWALK_ALLOWED_IMG_EXT);
      }

      if (aud) {
        preppedData.aud = fixCrosswalkPaths('aud', aud, CROSSWALK_ALLOWED_AUD_EXT);
      }

      if (vid) {
        preppedData.vid = fixCrosswalkPaths('vid', vid, CROSSWALK_ALLOWED_VID_EXT);
      }
