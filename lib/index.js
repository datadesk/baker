...
export class Baker extends EventEmitter {
  ...
  constructor({
    ...
    console.log({basepath: resolve('/', pathPrefix)})
  ...
  prepareCrosswalk(crossWalkRows) {
    ...
    return crossWalkRows.reduce((acc, crosswalkRow) => {
      ...
      if (!CROSSWALK_ALLOWED_ASSET_TYPES.includes(assetType)) {
        throw new Error(`Crosswalk: Unrecognized assetType: ${assetType} for asset ${assetName}`);
      }
      ...
      if (assetType === 'img' && !assetSources.jpg && !assetSources.png) {
        throw new Error('Crosswalk: Image assets require either a jpg or png file for basic compatibility.');
      }

      if (assetType === 'aud' && !assetSources.mp3) {
        throw new Error('Crosswalk: Audio asset type requires a mp3 file for basic compatibility');
      }

      if (assetType === 'vid' && !assetSources.mp4) {
        throw new Error('Crosswalk: Audio asset type requires a mp4 file for basic compatibility');
      }

      if (!preppedData.assets[assetType]) preppedData.assets[assetType] = {};
      preppedData.assets[assetType][assetName] = {};

      Object.entries(assetSources)
        .forEach(([ext, path]) => {
          const allowedExtensions = CROSSWALK_ALLOWED_EXTS[assetType];
          if (!allowedExtensions.includes(ext)) {
            console.warn(`Attribute: ${ext} not allowed for asset type: ${assetType}. Skipping.`);
            return;
          }

          preppedData.assets[assetType][assetName][ext] = this.getStaticPath(path);
        });
  ...
  async sendWebhookRequest() {
    ...
    const nunjucksInputParts = this.nunjucks.input.split('/');
    const projectSlug = nunjucksInputParts[nunjucksInputParts.length - 1];

    const embedsSlugs = Object.values(this.nunjucks.manifest).reduce(
      (result, path) => {
        const pathParts = path.split('/');
        const dir = pathParts[0];

        if (dir === 'embeds') {
          const embedSlug = pathParts[1];
          result.push(embedSlug);
        }
