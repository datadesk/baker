...
const CROSSWALK_ALLOWED_EXTS = {
  img: [ 'jpg', '.jpeg', 'png', '.gif', '.svg', 'avif', 'webp' ],
  aud: [ 'mp3' ],
  vid: [ 'mp4', 'webm' ]
};
  ...
  prepareCrosswalk(crossWalkRows) {
    ...
    return crossWalkRows.reduce((acc, crosswalkRow) => {
      ...
      if (!CROSSWALK_ALLOWED_ASSET_TYPES.includes(normalizedAssetType)) {
        throw new Error(`Crosswalk: Unrecognized assetType: ${normalizedAssetType} for asset ${normalizedAssetName}`);
      }
      ...
      if (normalizedAssetType === 'img' && !assetSources.jpg && !assetSources.png) {
      ...
      if (!preppedData.assets[normalizedAssetType]) preppedData.assets[normalizedAssetType] = {};
      ...
      Object.entries(assetSources)
        .forEach(([ext, path]) => {
          ...
          if (!allowedExtensions.includes(ext)) {
            console.warn(`Attribute: ${ext} not allowed for asset type: ${normalizedAssetType}. Skipping.`);
            return;
          }
