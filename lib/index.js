...
const { createSvelteBlock } = require('./blocks/svelte');
...
class Baker extends EventEmitter {
  ...
  constructor({
    ...
    // a svelte renderer on the server-side
    const svelteBlock = createSvelteBlock(this.input);

    this.nunjucks.addCustomTag('svelte', svelteBlock);
    ...
    this.nunjucks.addCustomFilter('log', (value) => {
      // log the input
      console.log(value);
