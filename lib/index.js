...
import { createStaticBlock, isFullUrl } from './blocks/static.js';
 ...
 * @property {(requestOptions: RequestInit) => RequestInit}  webHookPreflight
...
export class Baker extends EventEmitter {
  ...
  constructor({
    ...
    this.webHookPreflight = webHookPreflight;
  ...
  async sendWebhookRequest() {
    ...
    const projectName = nunjucksInputParts[nunjucksInputParts.length - 1];

    const embeds = Object.values(this.nunjucks.manifest).reduce(
      (result, path) => {
        const pathParts = path.split('/');
        const dir = pathParts[0];

        if (dir === 'embeds') {
          const fullEmbedUrl = new URL(path, this.basePath).toString();
          result.push(fullEmbedUrl);
        }
    ...
    // Run preflight handler if it is defined
    if (this.webHookPreflight && typeof this.webHookPreflight === 'function') {
      requestOptions = this.webHookPreflight(requestOptions);
    }

    await fetch(this.webHookUrl, requestOptions);
  }
  ...
  async bake() {
    ...
    if (this.webHookUrl && isFullUrl(this.webHookUrl)) {
