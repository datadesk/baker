...
// tags
const { createInjectTag } = require('./tags/inject');
const { createScriptTag } = require('./tags/script');
const { createStaticTag, createStaticAbsoluteTag } = require('./tags/static');
...
class Baker extends EventEmitter {
  ...
  constructor({
    ...
    this.sass.addFunction('static-url($file)', (sass) => ($file) =>
      new sass.types.String(`url(${staticTag($file.getValue())})`)
    );
  }
