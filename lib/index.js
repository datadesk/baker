...
export class Baker extends EventEmitter {
  ...
  async buildEmbedFallbacks() {
    const embedFiles = Object.values(this.nunjucks.manifest).filter((path) => path.includes('embeds'))
    ...
    for (const embedFilePath of embedFiles) {
      const fullPath = join(this.output, embedFilePath);
      const embedDir = fullPath.split('/').slice(0, -1).join('/');
      console.log(`Taking screenshot of: ${fullPath}`);

      const page = await browser.newPage();
      await page.goto(`file://${fullPath}`, { waitUntil: 'networkidle0' });

      const screenshotPath = join(embedDir, 'fallback.png');
