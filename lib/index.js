...
export class Baker extends EventEmitter {
  ...
  prepareCrosswalk(/** @type {string} */ crosswalkPath) {
    ...
    return function setCrosswalkPath(
      ...
      return data.map((d) => {
        const relativeStaticPath = this.getStaticPath(join(crosswalkPath, d.img));
        const absoluteUrlSrc = new URL(relativeStaticPath, removeTrailingSlash(this.domain)).href;
        if (d.img) {
          return {
            ...d,
            img: isProductionEnv ? absoluteUrlSrc : relativeStaticPath,
          };
        }
        return { ...d };
      });
