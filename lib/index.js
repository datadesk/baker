...
export class Baker extends EventEmitter {
  ...
  async buildEmbedFallbacks(local) {
    ...
    for (const embedFilePath of embedFiles) {
      const screenshotPath = `${local}/${embedFilePath}`;
      console.log(`Taking screenshot of: ${screenshotPath}`);

      const page = await browser.newPage();
      await page.goto(screenshotPath, {
  ...
  async screenshot() {
    ...
    await this.buildEmbedFallbacks(local);

    console.log(green('Screenshots built successfully! Closing server...'));
    await server.close();
  }
}
