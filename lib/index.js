...
// blocks
const { createAssetBlock } = require('./blocks/asset');
...
class Baker extends EventEmitter {
  ...
  constructor({
    ...
    this.sass.addFunction('asset-path($file)', (sass) => ($file, done) => {
      assetBlock($file.getValue()).then((value) => {
        done(new sass.types.String(value));
      });
    });

    this.sass.addFunction('asset-url($file)', (sass) => ($file, done) => {
      assetBlock($file.getValue()).then((value) => {
        done(new sass.types.String(`url(${value})`));
      });
    });
  }
