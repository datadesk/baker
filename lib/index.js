...
export class Baker extends EventEmitter {
   ...
   * @param {string} basePath - The base path for crosswalk images.
  ...
  prepareCrosswalk() {
    ...
    return function setCrosswalkDefaultPaths(
      ...
      const fixCrosswalkPaths = (obj, allowedExtensions) => {
        return Object.entries(obj).reduce((result, [ext, path]) => {
          if (!allowedExtensions.includes(ext)) return result;
      ...
      if (img) {
        preppedData.img = fixCrosswalkPaths(img, CROSSWALK_ALLOWED_IMG_EXT);
      }

      if (aud) {
        preppedData.aud = fixCrosswalkPaths(aud, CROSSWALK_ALLOWED_AUD_EXT);
      }

      if (vid) {
        preppedData.vid = fixCrosswalkPaths(vid, CROSSWALK_ALLOWED_VID_EXT);
      }
