...
export class Baker extends EventEmitter {
  ...
  async bakeEmbeds(filenames) {
    ...
    // Build map of html string identifiers to file paths
    for (const filename of filenames) {
      try {
        const filePath = resolve(__dirname, filename);
        const basePath = dirname(filePath);
        const bakerId = basename(basePath);

        fileMapper[bakerId] = `${bakerId}/index.html`;
      } catch (err) {
    ...
    const embedScriptContent = `
      module.exports = class BakerWebComponent extends HTMLElement {
        constructor() {
          super();
          this.fileMapper = {};

          this.populateFileMapper(BakerWebComponent.getBaseURL());
        }
        ...
        /**
        * Populate the fileMapper with the base URL and the file path
        *
        * @param {string} baseURL
        */
        populateFileMapper(baseURL) {

          const originalFileMapper = ${JSON.stringify(fileMapper)};
          for (const [key, value] of Object.entries(originalFileMapper)) {
            this.fileMapper[key] = baseURL + value;
          }
        }

        async connectedCallback() {
          const bakerId = this.getAttribute('id');
          const filePath = this.fileMapper[bakerId];

          if (!filePath) {
            console.error('Failed to find the file path for the element id: ', id);
            return;
          }
          ...
          template.innerHTML = doc.documentElement.outerHTML;
        ...
        static async parseHTML(htmlContent) {
          ...
          // Remove meta, title, and unwanted link tags
          const elementsToRemove = doc.querySelectorAll('meta, title, link[rel]:not([rel="stylesheet"])');
          elementsToRemove.forEach(el => el.remove());

          // intercept link rel stylesheet src to fix path
          const baseURL = BakerWebComponent.getBaseURL();
        ...
        static register() {
          ...
          // Wait for it
          return window.customElements.whenDefined(BakerWebComponent.customElementName);
        }

        /**
        * Parse the base URL from the embed.js script
        * @returns {string}
        */
        static getBaseURL() {
          // Find the embed.js script in the document
      ...
      `;
    const embedScriptPath = join('lib', 'components', 'web-component.js');
