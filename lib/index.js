...
export class Baker extends EventEmitter {
  ...
  prepareCrosswalk(crossWalkRows) {
    ...
    return crossWalkRows.reduce((acc, crosswalkRow) => {
      ...
      if (
        ...
        throw new Error(
          'Crosswalk: Image assets require either a jpg or png file for basic compatibility. If you attempted to load a GIF file, they require a png or jpeg fallback for accessibility reasons.'
        );
      }
      ...
      Object.entries(assetSources).forEach(([ext, path]) => {
        ...
        if (!path) {
          console.error(
            `Could not find static path for asset: ${normalizedAssetName} with the extension: ${ext}`
          );
          return;
        }
  ...
  async screenshot() {
    ...
    await this.buildEmbedFallbacks(baseUrl);
