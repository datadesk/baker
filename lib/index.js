...
class Bakery {
  ...
  async serve() {
    ...
    server.init(
      ...
      async err => {
        ...
        watch(
          path.join(this.input, '**/*.{sass,scss}'),
          {
  ...
  async bake() {
    ...
    const staticManifest = await this.sass.build();
    ...
    this.nunjucks.addCustomTag(
      'static',
      createStaticBlock(this.pathPrefix, staticManifest)
    );

    this.nunjucks.addCustomTag(
      'script',
      createScriptBlock(this.pathPrefix, scriptManifest)
    );
