...
// tags
import { createInjectTag } from './tags/inject.js';
import { createScriptTag } from './tags/script.js';
import { createStaticTag, createStaticAbsoluteTag } from './tags/static.js';
...
export class Baker extends EventEmitter {
  ...
  constructor({
    ...
    const injectTag = createInjectTag(this.output, [this.assets, this.sass]);

    this.nunjucks.addCustomTag('inject', injectTag);

    // create our static tag
    const staticTag = createStaticTag(this.input, this.pathPrefix, [
      this.assets,
      this.sass,
    ]);
    ...
    this.nunjucks.addCustomTag('static', staticTag);

    const staticAbsoluteTag = createStaticAbsoluteTag(
      this.input,
      this.domain,
      this.pathPrefix,
    ...
    this.nunjucks.addCustomTag('staticabsolute', staticAbsoluteTag);
    ...
    this.sass.addFunction('static-path($file)', (sass) => ($file) =>
      new sass.types.String(staticTag($file.getValue()))
    );

    this.sass.addFunction('static-url($file)', (sass) => ($file) => {
      // console.log('sass:', sass.types.String);
      // console.log('$file.getValue():', $file.getValue());
      return new sass.types.String(`url(${staticTag($file.getValue())})`);
    });
  }
  ...
  async bake() {
    ...
    await this.assets.build();
    console.log(this.assets.manifest);
