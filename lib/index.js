  ...
  prepareCrosswalk(/** @type {string} */ crosswalkPath) {
    ...
    return function setCrosswalkPath(
      ...
      return data.map((d) => {
        ...
        const absoluteImgSrc = new URL(staticImgPath, this.domain).href;
        const relativeImgSrc = this.getStaticPath(join('assets/img/', d.img));
        if (d.img) {
          return {
            ...d,
            img: isProductionEnv ? absoluteImgSrc : relativeImgSrc,
          };
        }
        return { ...d };
      });
