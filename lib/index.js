...
export class Baker extends EventEmitter {
  ...
  async bakeEmbeds() {
    ...
    // handle referencing the hashed web components file in the baker-embed.js
    const manifest = JSON.parse(await fs.readFile(join(outputDir, 'manifest.json'), 'utf8'));
    const webComponentsHashedFile = manifest['web-component.js'];
    const bakerEmbedPath = join(outputDir, 'baker-embed.js');
    let bakerEmbedCode = await fs.readFile(join(outputDir, 'baker-embed.js'), 'utf8');
    bakerEmbedCode = bakerEmbedCode.replace('__WEB_COMPONENTS_HASH__', webComponentsHashedFile);

    await fs.writeFile(bakerEmbedPath, bakerEmbedCode);
  }
